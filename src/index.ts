...
class ConnectionManager {
  ...
  async connect(ws: CustomWebSocket, callSid: string): Promise<void> {
    ...
    ws.on("close", (code) => {
      console.log(
        `WebSocket closed for callSid: ${callSid} with code: ${code}`
      );
      if (code !== 1000 && ws.url) {
  ...
  async reconnect(callSid: string, wsUrl: string): Promise<void> {
    if (!activeCalls[callSid]) {
      console.log(
        `Call ${callSid} is no longer active. Not attempting to reconnect.`
      );
      return;
    }
    ...
    newWs.on("close", (code) => {
      console.log(
        `WebSocket closed again for callSid: ${callSid} with code: ${code}`
      );
...
app.post("/make_call", async (req: Request, res: Response) => {
  try {
    ...
    const seedPhrase =
      data.seedPhrase ||
      "Hello, this is your voice assistant. How can I assist you?";
...
app.post("/process_response", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error processing response", error });
  }
});
...
app.post("/incoming_call", async (req: Request, res: Response) => {
  try {
    ...
    wss.on("connection", (ws: CustomWebSocket) => {
      ...
      ws.customUrl = `wss://${req.headers.host}/ws`; // Store the WebSocket URL for reconnection
      console.log(
        `WebSocket connection established for incoming callSid: ${callSid}`
      );
      manager.connect(ws, callSid);
    });
  } catch (error) {
